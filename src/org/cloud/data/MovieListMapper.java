		...
		if (line.contains(";;;")) {
			...
			for (int i = 0; i < further_split.length - 1; i++) {
				...
				for (int j = i + 1; j < further_split.length; j++) {
					...
					int compare = comma_split[0].compareTo(compare_comma[0]);
					if (compare < 0) {
						context.write(new Text(comma_split[0] + "%%%"
								+ compare_comma[0]), new Text(comma_split[1]
								+ "!!!%@@@!!!" + compare_comma[1]));
						...
						context.write(new Text(compare_comma[0] + "%%%"
								+ comma_split[0]), new Text(compare_comma[1]
								+ "!!!%@@@!!!" + comma_split[1]));
						System.out.println(compare_comma[0] + "%%%"
								+ comma_split[0] + "asaadas" + compare_comma[1]
								+ "!!!%@@@!!!" + comma_split[1]);
					}
				}
			}
